...
public class P15644 {
	...
	static class Unit {
		...
		public Unit(Point red, Point blue, int cnt, String command) {
			super();
			this.red = red;
			this.blue = blue;
			this.cnt = cnt;
	...
	public static void main(String[] args) throws IOException {
		...
		if (ans != Integer.MAX_VALUE) System.out.println(direction);
	}
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (newRed.equals(hole) && !newBlue.equals(hole)) {
					if (ans > unit.cnt + 1) {
						ans = unit.cnt + 1;
						direction = i == 0 ? unit.command + "U" : i == 1 ? unit.command + "D" : i == 2 ? unit.command + "L" : unit.command + "R";
					}
					//ans = Math.min(ans, unit.cnt + 1);
					continue;
				}
				
				q.offer(new Unit(newRed, newBlue, unit.cnt + 1, i == 0 ? unit.command + "U" : i == 1 ? unit.command + "D" : i == 2 ? unit.command + "L" : unit.command + "R"));
